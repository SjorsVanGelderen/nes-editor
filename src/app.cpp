...
glm::vec2 App::plotStart = glm::vec2(-1, -1); // This is not correct, it's actually still on the surface
...
AppStatus App::Start()
{
  ...
  const std::vector<const std::function<AppStatus()>> entityStarters =
    ...
    , ([]() -> AppStatus
        { 
          return buttonPencil->Start
            ( "tool_pencil"
            , 0
            , []() -> GLuint { return 0; }
            , []() -> void { App::SetTool(Tool::Pixel); }
            );
        }
      )
    , ([]() -> AppStatus
    ...
    , ([]() -> AppStatus 
        {
          return buttonRectangle->Start
            ...
            , []() -> GLuint 
               { 
                 return App::GetTool() == Tool::RectangleFrame ? 1
                      : App::GetTool() == Tool::RectangleFill  ? 2
                      : 0; 
               }
    ...
    , ([]() -> AppStatus 
        { 
          return buttonAbout->Start
            ( "about"
            , 4
            , []() -> GLuint { return 0; }
            , []() -> void { Debug::Log(LogLevel::Info, "Not implemented"); }
            ); 
        }
      )
    , ([]() -> AppStatus
        { 
          return buttonSave->Start
            ( "save"
            , 5
            , []() -> GLuint { return 0; }
            , []() -> void { Media::SaveCharacter(); }
            ); 
        }
      )
    , ([]() -> AppStatus 
        { 
          return buttonLoad->Start
            ( "load"
            , 6
            , []() -> GLuint { return 0; }
            , []() -> void { Media::LoadCharacter(); }
            ); 
        }
      )
  };
  ...
  Media::LoadSamples(); // TODO: Default palette should be moved to samples source instead

  // Start the main loop and eventually return the result
  return Update();
}
...
{    
  ...
  // TODO: Fix clicking logic, I can't switch between tools using the buttons right now
  if(!plotting && !*clickConsumed && m.x != -1) // m.x == -1 is blocking things
  {
...
{
  glClearColor(0.3f, 0.24f, 0.71f, 1.0f);
